...
module Reader : READER = struct
  ...
  and nt_string str =
    ...
    nt1 str
  and nt_vector str =
    let nt1 = caten (word "#(") nt_skip_star in
    let nt2 = star nt_sexpr in
    let nt3 = char ')' in
    let nt2 = caten nt1 (caten nt2 nt3) in
    let nt1 = pack nt2 (fun ((_, _), (exprs, _)) -> ScmVector(exprs)) in
  nt1 str
  and nt_list str =
    let nt1 = char '(' in
...
;;
  ...
  let rec macro_expand_letrec_ribs ribs =
    match ribs with
    | ScmPair(ScmPair(ScmSymbol(x),a),rest_ribs) -> 
      ScmPair(ScmPair(ScmSymbol(x), ScmPair(ScmPair(ScmSymbol("quote"), ScmPair(ScmSymbol("whatever"), ScmNil)), ScmNil)), (macro_expand_letrec_ribs rest_ribs))
    | _ -> ScmNil;;

  let rec macro_expand_letrec_body ribs body =
    match ribs with
    | ScmPair(ScmPair(ScmSymbol(x),a),rest_ribs) -> 
      ScmPair(ScmPair(ScmSymbol("set!"), ScmPair(ScmSymbol(x), a)), (macro_expand_letrec_body rest_ribs body))
    | _ -> body;;
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
       ...
       (ScmSymbol "let",
        ScmPair
         ((macro_expand_letrec_ribs ribs),
          (macro_expand_letrec_body ribs exprs))))
    ...
    let rec run params env = function
      ...
      | ScmLambda' (params', Opt opt, body) ->
        let label_loop_env = make_lambda_opt_loop_env ()
         ...
         and label_loop_exact = make_lambda_opt_loop()
         and label_loop_more_first = make_lambda_opt_loop()
         and label_loop_more_second = make_lambda_opt_loop()
         in
         (*Allocation of closure*)
         "\tmov rdi, (1 + 8 + 8)\t; sob closure\n"
         ^ "\tcall malloc\n"
         ^ "\tpush rax\n"
         (*Allocation of New-rib from old function*)
         ^ (Printf.sprintf "\tmov rdi, 8 * %d\t; new rib\n" params)
         ^ "\tcall malloc\n"
         ^ "\tpush rax\n"
         (*Allocation of Extended env*)
         ^ (Printf.sprintf "\tmov rdi, 8 * %d\t; extended env\n" (env + 1))
         ^ "\tcall malloc\n"
         ^ "\tmov rdi, ENV\n"
         (*Update Ext-env*)
